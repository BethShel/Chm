...
internal static class Program
{
    ...
    private static void Solve(int initN, double a, double b, double y0, double z0, double eps0)
    {
        ...
        {
            iterationsCount++;
            if(iterationsCount==29)
            {
                throw new ApplicationException("Количество шагов было превышено!");
            }
    ...
    static void drawingGraph(double[] x1, double[] y1, double[] z1, double[] x2, double[] y2, double[] z2, double[] x3, double[] y3, double[] z3)
    {
        try
        {
            ...
            File.WriteAllText(@"D:\Лиза\ВУЗ\ЧМ\second\second\Save\temp.json", json);
            var p = new Process();
            p.StartInfo = new ProcessStartInfo(@"D:\Лиза\ВУЗ\ЧМ\second\second\visualizationGraphs\visualizationGraphs.py")
            {
    ...
    private static Solution GetSolution(double h, double y0, double z0, double[] x, int pointsQuantity, ref double[] yi, ref double[] zi)
    {
        ...
        for (var i = 1; i < pointsQuantity; i++)
        {
            ...
            var k2 = h * Y(x[i - 1] + h / 2, yprev + k1 / 2, zprev + l1 / 2);
            var l2 = h * Z(x[i - 1] + h / 2, yprev + k1 / 2, zprev + l1 / 2);

            var k3 = h * Y(x[i - 1] + h / 2, yprev + k2 / 2, zprev + l2 / 2);
            var l3 = h * Z(x[i - 1] + h / 2, yprev + k2 / 2, zprev + l2 / 2);

            var k4 = h * Y(x[i - 1] + h, yprev + k3, zprev + l3);
            var l4 = h * Z(x[i - 1] + h, yprev + k3, zprev + l3);

            yprev = yprev + (k1 + 2 * k2 + 2 * k3 + k4) / 6;
            zprev = zprev + (l1 + 2 * l2 + 2 * l3 + l4) / 6;
            yi[i] = yprev;
            zi[i] = zprev;
        }
